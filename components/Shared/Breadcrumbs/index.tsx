...
export default function Breadcrumbs(): ReactElement {
    ...
    return (
        <div className={styles.breadcrumbs}>
            {project_client && project_client.length > 0 && project_client[0].text && <span>{project_client[0].text}</span>}

            {project_title && project_title.length > 0 && project_title[0].text && <span className={styles.title}>{project_title[0].text}</span>}

            {chapter_name && chapter_name.length > 0 && chapter_name[0].text && <span className={styles.chapter}>{chapter_name[0].text}</span>}
        </div>
    );
}
